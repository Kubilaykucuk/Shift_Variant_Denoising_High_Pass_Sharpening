...
// Function to apply the bilateral filter manually
cv::Mat manualBilateralFilter(const cv::Mat& src, int d, double sigmaColor, double sigmaSpace) {
    ...
    std::cout << "Actual Execution Time: " << actual_time.count() << " seconds" << std::endl;

    return dst;
}

// Function to apply the high-pass filter manually using your conv2D function
...
cv::Mat processImage(const cv::Mat& src) {
    ...
    float tau = 0.25f;
    if (src.channels() == 1) {
        denoised = totalVariationDenoising(src, lambda, iterations, tau);
    } else {
...
int main() {
    ...
    std::string imagePath = "monarch.png";
    ...
    cv::GaussianBlur(img,blur,cv::Size(kernel_size,kernel_size),sigma);

    auto start = std::chrono::high_resolution_clock::now();
    cv::Mat processedImg = processImage(blur);
    auto end = std::chrono::high_resolution_clock::now();
    
    std::chrono::duration<double> actual_time = end - start;
    cv::Mat processedImgFloat, imgFloat;
    processedImg.convertTo(processedImgFloat, CV_32F);
    blur.convertTo(imgFloat, CV_32F);
    double ssim_value = computeSSIM(processedImgFloat, imgFloat);
    
    std::cout << "Actual Execution Time: " << actual_time.count() << " seconds" << std::endl;
    std::cout << "FPS: " << 1 / actual_time.count() << std::endl;
    std::cout << "SSIM Score: " << ssim_value << std::endl;
    
    blur.convertTo(blur, CV_8U);
    processedImg.convertTo(processedImg, CV_8U);
    int R = 10;
    int x = 64 - R;
    int y = 50 - R;
    cv::Rect roi(x, y, 2 * R + 1, 2 * R + 1);

    cv::Mat imgROI = blur(roi);
    cv::Mat processedImgROI = processedImg(roi);
    double ksize = 8.0;
    cv::resize(imgROI,imgROI, cv::Size(),ksize,ksize);
    cv::resize(processedImgROI,processedImgROI, cv::Size(),ksize,ksize);


    cv::imshow("ROI1", imgROI);
    cv::imshow("ROI2", processedImgROI);
    cv::imshow("Original Image", blur);
